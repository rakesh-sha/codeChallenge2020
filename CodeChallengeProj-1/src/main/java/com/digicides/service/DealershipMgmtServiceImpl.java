...
public class DealershipMgmtServiceImpl implements DealershipMgmtService<DealerDto, LatLng> {
	 ...
	 * Logger
	 */
	private Logger logger = Logger.getLogger(DealershipMgmtServiceImpl.class);
	...
	public DealerDto registerDealer(DealerDto dto) {
		...
		logger.info(dealerEntity+" is added ");
		return dto;
	}
	...
	public List<DealerDto> fetchAllDealerDetails() {
		...
		data = repo.findAll();
		/*
		 * convirting list of entity to list of dto by traversing throgh each entity and
		 * copieng to dto at last adding to the dtolist
		 */ data.forEach(entity -> {
		...
		logger.info("total  " + dtoList.size() + "no of record is passed to controller ");
		return dtoList;
	}
	...
	public DealerDto getDealer(LatLng geocode) {
		...
		dto.setAddress(addr);
		logger.info("dealer for geocode " + geocode + " is " + dto.getName());
		return dto;
	}
	...
	public Dealer findNearest(LatLng geocode) {
		...
		logger.info(" Dealer " + nearestDealer.getName() + " found at " + nearestDist + " KM");
